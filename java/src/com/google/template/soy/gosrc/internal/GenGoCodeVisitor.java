...
class GenGoCodeVisitor extends AbstractSoyNodeVisitor<List<Pair<String, String>>> {
  ...
  @Override protected List<Pair<String, String>> getResult() {
    ...
    if(node instanceof CallNode) {
      ...
      for(SoyNode n : node.getChildren()) {
        if(n instanceof ParentSoyNode) {
          elems.addAll(getAllExternalCallLibraryNames((ParentSoyNode<? extends Node>)n));
        }
      }
    }
    return elems;
  ...
  @Override protected void visitInternal(ForeachNode node) {
    ...
    GoExpr dataRefGoExpr =
        translateToGoExprVisitorFactory.create(localVarTranslations).exec(node.getDataRef());
    goCodeBuilder.appendLine(
        listVarName,
